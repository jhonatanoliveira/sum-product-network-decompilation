...
from models import SumProductNetwork, SubplotDrawer
...
if __name__ == "__main__":
    ...
    parser.add_argument("--size", type=int,
                        help="Consider image size x size",
                        required=True)
    ...
    var_cardinalities = {i: 2 for i in range(height * width)}
    spn = SumProductNetwork(dag, var_cardinalities)
    bn = decompile(spn, comp_assumption="ve")

    plotter = SubplotDrawer("Decompiling P&D Structure")
    plotter.add(spn, "SPN")
    plotter.add(bn, "BN")
    plotter.plot()
